...
ggplot() + 
  ...
  scale_fill_viridis_c(
    ...
    name = "% A*A*A or Above",
    direction = -1
  ) + theme_void() + coord_sf(xlim = c(-8, 2), ylim = c(49.5, 59))
...
# Main map
main <- ggplot() + 
  geom_sf(data = LAD.Map, aes(fill = `AtAndAboveA*A*A`), colour = "grey40", linewidth = 0.05) +
  scale_fill_viridis_c(option = "magma", na.value = "#f0f0f0", name = "% A*A*A+", direction = -1) +
  coord_sf(xlim = c(-8, 2), ylim = c(49.5, 59)) +
  theme_void()

# London inset
london_map <- LAD.Map %>% filter(str_starts(LAD24CD, "E09"))

london <- ggplot() + 
  geom_sf(data = london_map, aes(fill = `AtAndAboveA*A*A`), colour = "grey40", linewidth = 0.1) +
  scale_fill_viridis_c(option = "magma", na.value = "#f0f0f0", direction = -1, guide = "none") +
  theme_void() +
...
# Mets = createEntrantMap(region="Mets")
# NonMets = createEntrantMap(region="Non Mets")
# Unitary = createEntrantMap(region="Unitary")
